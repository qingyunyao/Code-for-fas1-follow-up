...
/*Purpose....: generate table 1 (baseline characteristics)*/
/* Source: fas1_fu_table1_Qingyun_20231205.sas (upstream analysis, unmodified).
   newcoh.analysis_0205 / newcoh.analysis_1020 are supplied by the bundle
   autoexec as small synthetic stand-ins. The upstream leading
   `proc datasets kill;` (which would empty the synthetic library) and the
   trailing ODS RTF export to a hardcoded external Windows path are omitted
   so the run is self-contained. Analysis code otherwise unchanged. */
