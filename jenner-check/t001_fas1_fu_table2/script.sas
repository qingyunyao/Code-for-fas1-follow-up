...
/* Source: fas1_fu_table2_Qingyun_20231205.sas (upstream, unmodified analysis).
   The two source datasets are provided by the bundle autoexec as a small
   synthetic newcoh library; the external ODS RTF export to a hardcoded
   Windows path at the end of the original file is omitted so the run is
   self-contained. Everything below is the author's original code. */
